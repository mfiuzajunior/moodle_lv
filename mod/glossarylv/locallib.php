...
/**
 * class to handle exporting an entire glossarylv database
 */
class glossarylv_full_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        global $DB;
        if (!$this->cm = get_coursemodule_from_id('glossarylv', $this->id)) {
            throw new portfolio_caller_exception('invalidid', 'glossarylv');
        }
        if (!$this->glossarylv = $DB->get_record('glossarylv', array('id' => $this->cm->instance))) {
            throw new portfolio_caller_exception('invalidid', 'glossarylv');
        }
        $entries = $DB->get_records('glossarylv_entries', array('glossarylvid' => $this->glossarylv->id));
        list($where, $params) = $DB->get_in_or_equal(array_keys($entries));

        $aliases = $DB->get_records_select('glossarylv_alias', 'entryid ' . $where, $params);
        $categoryentries = $DB->get_records_sql('SELECT ec.entryid, c.name FROM {glossarylv_entries_categ} ec
            JOIN {glossarylv_categories} c
        ...
        foreach (array_keys($entries) as $entry) {
            $this->keyedfiles[$entry] = array_merge(
                $fs->get_area_files($context->id, 'mod_glossarylv', 'attachment', $entry, "timemodified", false),
                $fs->get_area_files($context->id, 'mod_glossarylv', 'entry', $entry, "timemodified", false)
            );
    ...
    public function prepare_package() {
        ...
        if ($this->get('exporter')->get('formatclass') == PORTFOLIO_FORMAT_SPREADSHEET) {
            $csv = glossarylv_generate_export_csv($entries, $aliases, $categories);
            ...
            foreach ($entries as $e) {
                $content = glossarylv_entry_portfolio_caller::entry_content(
                    $this->course,
                    $this->cm,
                    $this->glossarylv,
                    $e,
                ...
                $entry = new portfolio_format_leap2a_entry('glossarylventry' . $e->id, $e->concept, 'entry', $content);
                ...
                if (!empty($this->keyedfiles[$e->id])) {
                    $writer->link_files($entry, $this->keyedfiles[$e->id], 'glossarylventry' . $e->id . 'file');
            ...
            $selection = new portfolio_format_leap2a_entry('wholeglossarylv' . $this->glossarylv->id, get_string('modulename', 'glossarylv'), 'selection');
    ...
    public function check_permissions() {
        return has_capability('mod/glossarylv:export', context_module::instance($this->cm->id));
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'glossarylv');
    }
...
/**
 * class to export a single glossarylv entry
 *
 * @package   mod_glossarylv
 * @copyright 1999 onwards Martin Dougiamas  {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class glossarylv_entry_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function load_data() {
        ...
        if ($this->entryid) {
            if (!$this->entry = $DB->get_record('glossarylv_entries', array('id' => $this->entryid))) {
                throw new portfolio_caller_exception('noentry', 'glossarylv');
            }
            // in case we don't have USER this will make the entry be printed
            $this->entry->approved = true;
        }
        $this->categories = $DB->get_records_sql('SELECT ec.entryid, c.name FROM {glossarylv_entries_categ} ec
            JOIN {glossarylv_categories} c
            ON c.id = ec.categoryid
            WHERE ec.entryid = ?', array($this->entryid));
        $context = context_module::instance($this->cm->id);
        if ($this->entry->sourceglossarylvid == $this->cm->instance) {
            if ($maincm = get_coursemodule_from_instance('glossarylv', $this->entry->glossarylvid)) {
                $context = context_module::instance($maincm->id);
            }
        }
        $this->aliases = $DB->get_record('glossarylv_alias', array('entryid'=>$this->entryid));
        $fs = get_file_storage();
        $this->multifiles = array_merge(
            $fs->get_area_files($context->id, 'mod_glossarylv', 'attachment', $this->entry->id, "timemodified", false),
            $fs->get_area_files($context->id, 'mod_glossarylv', 'entry', $this->entry->id, "timemodified", false)
        );
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cm->id);
        return has_capability('mod/glossarylv:exportentry', $context)
            || ($this->entry->userid == $this->user->id && has_capability('mod/glossarylv:exportownentry', $context));
    }
    ...
    public function prepare_package() {
        ...
        $format = $this->exporter->get('format');
        $content = self::entry_content($this->course, $this->cm, $this->glossarylv, $this->entry, $this->aliases, $format);
        ...
        if ($this->exporter->get('formatclass') === PORTFOLIO_FORMAT_PLAINHTML) {
            ...
            $entry = new portfolio_format_leap2a_entry('glossarylventry' . $this->entry->id, $this->entry->concept, 'entry', $content);
            ...
            throw new portfolio_caller_exception('unexpected_format_class', 'glossarylv');
        }
    }
     ...
     * for both this class and the full glossarylv exporter
     ...
     * @return string
     */
    public static function entry_content($course, $cm, $glossarylv, $entry, $aliases, $format) {
        global $OUTPUT, $DB;
        $entry = clone $entry;
        ...
        $output = '<table class="glossarylvpost dictionary" cellspacing="0">' . "\n";
        ...
        $output .= portfolio_rewrite_pluginfile_urls($entry->definition, $context->id, 'mod_glossarylv', 'entry', $entry->id, $format);
        ...
        if (!empty($aliases)) {
            ...
            $output .= get_string($key, 'glossarylv') . ': ';
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class glossarylv_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false) {
        ...
        $sql = 'SELECT DISTINCT f.itemid, ge.concept
                  ...
                  JOIN {glossarylv_entries} ge ON (ge.glossarylvid = g.id AND ge.id = f.itemid)
                 WHERE f.contextid = :contextid
                  AND f.component = :component
                  AND f.filearea = :filearea';
        $params = array(
            'modulename' => 'glossarylv',
        ...
        foreach ($rs as $record) {
            if ($child = $this->browser->get_file_info($this->context, 'mod_glossarylv', $this->filearea, $record->itemid)) {
                $children[] = $child;
            }
...
/**
 * Returns glossarylv entries tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_glossarylv/glossarylv_entries to search for glossarylv entries
 ...
 */
function mod_glossarylv_get_tagged_entries($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = 1, $page = 0) {
    global $OUTPUT;
    $perpage = $exclusivemode ? 20 : 5;

    // Build the SQL query.
    $ctxselect = context_helper::get_preload_record_columns_sql('ctx');
    $query = "SELECT ge.id, ge.concept, ge.glossarylvid, ge.approved, ge.userid,
                    cm.id AS cmid, c.id AS courseid, c.shortname, c.fullname, $ctxselect
                FROM {glossarylv_entries} ge
                JOIN {glossarylv} g ON g.id = ge.glossarylvid
    ...
    $params = array('itemtype' => 'glossarylv_entries', 'tagid' => $tag->id, 'component' => 'mod_glossarylv',
    ...
    $builder = new core_tag_index_builder('mod_glossarylv', 'glossarylv_entries', $query, $params, $page * $perpage, $perpage + 1);
            ...
            if ($taggeditem->courseid == $courseid) {
                ...
                if (($cm = $modinfo->get_cm($taggeditem->cmid)) && $cm->uservisible) {
                    if ($taggeditem->approved) {
                        ...
                        $accessible = has_capability('mod/glossarylv:approve', context_module::instance($cm->id));
                    }
                }
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $pageurl = new moodle_url('/mod/glossarylv/showentry.php', array('eid' => $item->id, 'displayformat' => 'dictionary'));
        ...
        return new core_tag\output\tagindex($tag, 'mod_glossarylv', 'glossarylv_entries', $content,
