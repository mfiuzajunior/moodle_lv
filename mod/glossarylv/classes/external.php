...
/**
 * Glossarylv module external API.
 *
...
/**
 * Glossarylv module external functions.
 *
 * @package    mod_glossarylv
 * @category   external
 * @copyright  2015 Costantino Cito <ccito@cvaconsulting.com>
 ...
 * @since      Moodle 3.1
 */
class mod_glossarylv_external extends external_api {

    /**
     * Get the browse modes from the display format.
     *
     * This returns some of the terms that can be used when reporting a glossarylv being viewed.
     *
     * @param  string $format The display format of the glossarylv.
    ...
    protected static function get_browse_modes_from_display_format($format) {
        ...
        $formats = array();
        $dp = $DB->get_record('glossarylv_formats', array('name' => $format), '*', IGNORE_MISSING);
        if ($dp) {
            $formats = glossarylv_get_visible_tabs($dp);
        }
    ...
    protected static function get_entry_return_structure($includecat = false) {
        $params = array(
            ...
            'glossarylvid' => new external_value(PARAM_INT, 'The glossarylv ID'),
            ...
            'sourceglossarylvid' => new external_value(PARAM_INT, 'The source glossarylv ID'),
        ...
        if ($includecat) {
            $params['categoryid'] = new external_value(PARAM_INT, 'The category ID. This may be' .
                ' \''. GLOSSARYLV_SHOW_NOT_CATEGORISED . '\' when the entry is not categorised', VALUE_DEFAULT,
                GLOSSARYLV_SHOW_NOT_CATEGORISED);
    ...
    protected static function fill_entry_details($entry, $context) {
        ...
        list($entry->definition, $entry->definitionformat) = external_format_text($entry->definition, $entry->definitionformat,
            $context->id, 'mod_glossarylv', 'entry', $entry->id);
        ...
        if ($entry->attachment) {
            $entry->attachments = external_util::get_area_files($context->id, 'mod_glossarylv', 'attachment', $entry->id);
        }
        $definitioninlinefiles = external_util::get_area_files($context->id, 'mod_glossarylv', 'entry', $entry->id);
    ...
    /**
     * Validate a glossarylv via ID.
     *
     * @param  int $id The glossarylv ID.
     * @return array Contains glossarylv, context, course and cm.
     */
    protected static function validate_glossarylv($id) {
        global $DB;
        $glossarylv = $DB->get_record('glossarylv', array('id' => $id), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($glossarylv, 'glossarylv');
        $context = context_module::instance($cm->id);
        self::validate_context($context);
        return array($glossarylv, $context, $course, $cm);
    }
    ...
    public static function get_glossaries_by_courses($courseids = array()) {
        ...
        if (!empty($courseids)) {
            ...
            $glossaries = get_all_instances_in_courses('glossarylv', $courses);
            foreach ($glossaries as $glossarylv) {
                $context = context_module::instance($glossarylv->coursemodule);
                $glossarylv->name = external_format_string($glossarylv->name, $context->id);
                list($glossarylv->intro, $glossarylv->introformat) = external_format_text($glossarylv->intro, $glossarylv->introformat,
                    $context->id, 'mod_glossarylv', 'intro', null);
                $glossarylv->introfiles = external_util::get_area_files($context->id, 'mod_glossarylv', 'intro', false, false);

                // Make sure we have a number of entries per page.
                if (!$glossarylv->entbypage) {
                    $glossarylv->entbypage = $CFG->glossarylv_entbypage;
                }

                // Add the list of browsing modes.
                if (!isset($modes[$glossarylv->displayformat])) {
                    $modes[$glossarylv->displayformat] = self::get_browse_modes_from_display_format($glossarylv->displayformat);
                }
                $glossarylv->browsemodes = $modes[$glossarylv->displayformat];
                $glossarylv->canaddentry = has_capability('mod/glossarylv:write', $context) ? 1 : 0;
            }
        }
        ...
        return new external_single_structure(array(
            'glossaries' => new external_multiple_structure(
                new external_single_structure(array(
                    'id' => new external_value(PARAM_INT, 'Glossarylv id'),
                    'coursemodule' => new external_value(PARAM_INT, 'Course module id'),
                    'course' => new external_value(PARAM_INT, 'Course id'),
                    'name' => new external_value(PARAM_RAW, 'Glossarylv name'),
                    'intro' => new external_value(PARAM_RAW, 'The Glossarylv intro'),
                    ...
                    'mainglossarylv' => new external_value(PARAM_INT, 'If enabled this glossarylv is a main glossarylv.'),
                    'showspecial' => new external_value(PARAM_INT, 'If enabled, participants can browse the glossarylv by' .
                        ' special characters, such as @ and #'),
                    'showalphabet' => new external_value(PARAM_INT, 'If enabled, participants can browse the glossarylv by' .
                    ...
                    'allowcomments' => new external_value(PARAM_INT, 'If enabled, all participants with permission to' .
                        ' create comments will be able to add comments to glossarylv entries'),
                    'allowprintview' => new external_value(PARAM_INT, 'If enabled, students are provided with a link to a' .
                        ' printer-friendly version of the glossarylv. The link is always available to teachers'),
                    'usedynalink' => new external_value(PARAM_INT, 'If site-wide glossarylv auto-linking has been enabled' .
                    ...
                    'approvaldisplayformat' => new external_value(PARAM_TEXT, 'When approving glossarylv items you may wish' .
                        ' to use a different display format'),
                    'globalglossarylv' => new external_value(PARAM_INT, ''),
                    ...
                    'rssarticles' => new external_value(PARAM_INT, 'This setting specifies the number of glossarylv entry' .
     ...
     * @since Moodle 3.1
     */
    public static function view_glossarylv_parameters() {
        return new external_function_parameters(array(
            'id' => new external_value(PARAM_INT, 'Glossarylv instance ID'),
            'mode' => new external_value(PARAM_ALPHA, 'The mode in which the glossarylv is viewed'),
        ));
    }
     ...
     *
     * @param int $id The glossarylv instance ID.
     * @param string $mode The view mode.
     * @return array of warnings and status result
     * @since Moodle 3.1
     * @throws moodle_exception
     */
    public static function view_glossarylv($id, $mode) {
        $params = self::validate_parameters(self::view_glossarylv_parameters(), array(
            'id' => $id,
            'mode' => $mode
        ));
        $id = $params['id'];
        ...
        // Get and validate the glossarylv.
        list($glossarylv, $context, $course, $cm) = self::validate_glossarylv($id);
     ...
     * @since Moodle 3.1
     */
    public static function view_glossarylv_returns() {
        return new external_single_structure(array(
    ...
    public static function view_entry_parameters() {
        return new external_function_parameters(array(
            'id' => new external_value(PARAM_INT, 'Glossarylv entry ID'),
        ));
    }
    ...
    public static function view_entry($id) {
        ...
        // Get and validate the glossarylv.
        $entry = $DB->get_record('glossarylv_entries', array('id' => $id), '*', MUST_EXIST);
        list($glossarylv, $context, $course, $cm) = self::validate_glossarylv($entry->glossarylvid);

        if (!glossarylv_can_view_entry($entry, $cm)) {
            throw new invalid_parameter_exception('invalidentry');
        }

        // Trigger view.
        glossarylv_entry_view($entry, $context);
    ...
    /**
     * Browse a glossarylv entries by letter.
     *
     * @param int $id The glossarylv ID.
    ...
    public static function get_entries_by_letter($id, $letter, $from, $limit, $options) {
        ...
        // Get and validate the glossarylv.
        list($glossarylv, $context) = self::validate_glossarylv($id);

        // Validate the mode.
        $modes = self::get_browse_modes_from_display_format($glossarylv->displayformat);
        if (!in_array('letter', $modes)) {
            throw new invalid_parameter_exception('invalidbrowsemode');
        }

        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_letter($glossarylv, $context, $letter, $from, $limit, $options);
    ...
    /**
     * Browse a glossarylv entries by date.
     *
     * @param int $id The glossarylv ID.
    ...
    public static function get_entries_by_date($id, $order, $sort, $from, $limit, $options) {
        ...
        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_date($glossarylv, $context, $order, $sort, $from, $limit, $options);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            $entries[] = $record;
        }
    ...
    public static function get_categories_parameters() {
        return new external_function_parameters(array(
            'id' => new external_value(PARAM_INT, 'The glossarylv ID'),
            'from' => new external_value(PARAM_INT, 'Start returning records from here', VALUE_DEFAULT, 0),
            'limit' => new external_value(PARAM_INT, 'Number of records to return', VALUE_DEFAULT, 20)
        ));
    }
    ...
    /**
     * Get the categories of a glossarylv.
     *
     * @param int $id The glossarylv ID.
     * @param int $from Start returning records from here.
     * @param int $limit Number of records to return.
    ...
    public static function get_categories($id, $from, $limit) {
        ...
        $categories = array();
        list($records, $count) = glossarylv_get_categories($glossarylv, $from, $limit);
    ...
    public static function get_entries_by_category_parameters() {
        return new external_function_parameters(array(
            'id' => new external_value(PARAM_INT, 'The glossarylv ID.'),
            'categoryid' => new external_value(PARAM_INT, 'The category ID. Use \'' . GLOSSARYLV_SHOW_ALL_CATEGORIES . '\' for all' .
                ' categories, or \'' . GLOSSARYLV_SHOW_NOT_CATEGORISED . '\' for uncategorised entries.'),
            'from' => new external_value(PARAM_INT, 'Start returning records from here', VALUE_DEFAULT, 0),
            'limit' => new external_value(PARAM_INT, 'Number of records to return', VALUE_DEFAULT, 20),
            'options' => new external_single_structure(array(
                'includenotapproved' => new external_value(PARAM_BOOL, 'When false, includes the non-approved entries created by' .
    ...
    /**
     * Browse a glossarylv entries by category.
     *
     * @param int $id The glossarylv ID.
    ...
    public static function get_entries_by_category($id, $categoryid, $from, $limit, $options) {
        ...
        if (in_array($categoryid, array(GLOSSARYLV_SHOW_ALL_CATEGORIES, GLOSSARYLV_SHOW_NOT_CATEGORISED))) {
            // All good.
        } else if (!$DB->record_exists('glossarylv_categories', array('id' => $categoryid, 'glossarylvid' => $id))) {
            throw new invalid_parameter_exception('invalidcategory');
        }

        // Fetching the entries.
        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_category($glossarylv, $context, $categoryid, $from, $limit, $options);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            if ($record->categoryid === null) {
                $record->categoryid = GLOSSARYLV_SHOW_NOT_CATEGORISED;
            }
    ...
    /**
     * Get the authors of a glossarylv.
     *
     * @param int $id The glossarylv ID.
     * @param int $from Start returning records from here.
     * @param int $limit Number of records to return.
    ...
    public static function get_authors($id, $from, $limit, $options) {
        ...
        // Fetching the entries.
        list($users, $count) = glossarylv_get_authors($glossarylv, $context, $limit, $from, $options);
    ...
    /**
     * Browse a glossarylv entries by author.
     *
     * @param int $id The glossarylv ID.
     * @param string $letter A letter, or a special keyword.
    ...
    public static function get_entries_by_author($id, $letter, $field, $sort, $from, $limit, $options) {
        ...
        // Fetching the entries.
        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_author($glossarylv, $context, $letter, $field, $sort, $from, $limit,
            $options);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            $entries[] = $record;
    ...
    public static function get_entries_by_author_id($id, $authorid, $order, $sort, $from, $limit, $options) {
        ...
        // Fetching the entries.
        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_author_id($glossarylv, $context, $authorid, $order, $sort, $from,
            $limit, $options);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            $entries[] = $record;
    ...
    /**
     * Browse a glossarylv entries using the search.
     *
     * @param int $id The glossarylv ID.
    ...
    public static function get_entries_by_search($id, $query, $fullsearch, $order, $sort, $from, $limit, $options) {
        ...
        // Fetching the entries.
        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_search($glossarylv, $context, $query, $fullsearch, $order, $sort, $from,
            $limit, $options);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            $entries[] = $record;
    ...
    /**
     * Browse a glossarylv entries using a term matching the concept or alias.
     *
     * @param int $id The glossarylv ID.
    ...
    public static function get_entries_by_term($id, $term, $from, $limit, $options) {
        ...
        // Fetching the entries.
        $entries = array();
        list($records, $count) = glossarylv_get_entries_by_term($glossarylv, $context, $term, $from, $limit, $options);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            $entries[] = $record;
        }
    ...
    public static function get_entries_to_approve($id, $letter, $order, $sort, $from, $limit) {
        ...
        // Fetching the entries.
        $entries = array();
        list($records, $count) = glossarylv_get_entries_to_approve($glossarylv, $context, $letter, $order, $sort, $from, $limit);
        foreach ($records as $key => $record) {
            self::fill_entry_details($record, $context);
            $entries[] = $record;
        }
    ...
    public static function get_entry_by_id($id) {
        ...
        // Get and validate the glossarylv.
        $entry = $DB->get_record('glossarylv_entries', array('id' => $id), '*', MUST_EXIST);
        list($glossarylv, $context) = self::validate_glossarylv($entry->glossarylvid);

        if (empty($entry->approved) && $entry->userid != $USER->id && !has_capability('mod/glossarylv:approve', $context)) {
            throw new invalid_parameter_exception('invalidentry');
        }

        $entry = glossarylv_get_entry_by_id($id);
    ...
    public static function add_entry_parameters() {
        return new external_function_parameters(array(
            'glossarylvid' => new external_value(PARAM_INT, 'Glossarylv id'),
            'concept' => new external_value(PARAM_TEXT, 'Glossarylv concept'),
            'definition' => new external_value(PARAM_RAW, 'Glossarylv concept definition'),
            'definitionformat' => new external_format_value('definition'),
    ...
    /**
     * Add a new entry to a given glossarylv.
     *
     * @param int $glossarylvid the glossarylv id
     * @param string $concept    the glossarylv concept
     ...
     * @throws invalid_parameter_exception
     */
    public static function add_entry($glossarylvid, $concept, $definition, $definitionformat, $options = array()) {
        global $CFG;

        $params = self::validate_parameters(self::add_entry_parameters(), array(
            'glossarylvid' => $glossarylvid,
            'concept' => $concept,
        ...
        // Get and validate the glossarylv.
        list($glossarylv, $context, $course, $cm) = self::validate_glossarylv($params['glossarylvid']);
        require_capability('mod/glossarylv:write', $context);

        if (!$glossarylv->allowduplicatedentries) {
            if (glossarylv_concept_exists($glossarylv, $params['concept'])) {
                throw new moodle_exception('errconceptalreadyexists', 'glossarylv');
            }
        }
        ...
        $entry->id = null;
        $entry->aliases = '';
        $entry->usedynalink = $CFG->glossarylv_linkentries;
        $entry->casesensitive = $CFG->glossarylv_casesensitive;
        $entry->fullmatch = $CFG->glossarylv_fullmatch;
        ...
        $entry = glossarylv_edit_entry($entry, $course, $cm, $glossarylv, $context);
    ...
    public static function add_entry_returns() {
        return new external_single_structure(array(
            'entryid' => new external_value(PARAM_INT, 'New glossarylv entry ID'),
            'warnings' => new external_warnings()
        ));
    }
