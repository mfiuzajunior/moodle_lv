...
/**
 * Entry caching for glossarylv filter.
 *
...
namespace mod_glossarylv\local;
defined('MOODLE_INTERNAL') || die();

/**
 * Concept caching for glossarylv filter.
 *
 * @package    mod_glossarylv
 * @copyright  2014 Petr Skoda
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class concept_cache {
    ...
    public static function cm_updated(\core\event\course_module_updated $event) {
        if ($event->other['modulename'] !== 'glossarylv') {
            return;
        }
    ...
    public static function reset_caches($phpunitreset = false) {
        ...
        $cache = \cache::make('mod_glossarylv', 'concepts');
        $cache->purge();
    }
    ...
    /**
     * Utility method to purge caches related to given glossarylv.
     * @param \stdClass $glossarylv
     */
    public static function reset_glossarylv($glossarylv) {
        if (!$glossarylv->usedynalink) {
            return;
        }
        self::reset_course_muc($glossarylv->course);
        if ($glossarylv->globalglossarylv) {
    ...
    protected static function fetch_concepts(array $glossaries) {
        ...
        $glossarylvlist = implode(',', $glossaries);

        $sql = "SELECT id, glossarylvid, concept, casesensitive, 0 AS category, fullmatch
                  FROM {glossarylv_entries}
                 WHERE glossarylvid IN ($glossarylvlist) AND usedynalink = 1 AND approved = 1

                 UNION

                SELECT id, glossarylvid, name AS concept, 1 AS casesensitive, 1 AS category, 1 AS fullmatch
                  FROM {glossarylv_categories}
                 WHERE glossarylvid IN ($glossarylvlist) AND usedynalink = 1

                UNION

                SELECT ge.id, ge.glossarylvid, ga.alias AS concept, ge.casesensitive, 0 AS category, ge.fullmatch
                  FROM {glossarylv_alias} ga
                  JOIN {glossarylv_entries} ge ON (ga.entryid = ge.id)
                 WHERE ge.glossarylvid IN ($glossarylvlist) AND ge.usedynalink = 1 AND ge.approved = 1";
        ...
        foreach ($rs as $concept) {
            ...
            $concepts[$concept->glossarylvid][] = $concept;
        }
        $rs->close();
    ...
    protected static function get_course_concepts($courseid) {
        ...
        $cminfos = $modinfo->get_instances_of('glossarylv');
        if (!$cminfos) {
        ...
        if (is_array($data)) {
            ...
            $sql = "SELECT g.id, g.name
                      ...
                      JOIN {modules} m ON (m.name = 'glossarylv' AND m.id = cm.module)
                     WHERE g.usedynalink = 1 AND g.course = :course AND cm.visible = 1 AND m.visible = 1
                  ORDER BY g.globalglossarylv, g.id";
        ...
        // Verify access control to glossarylv instances.
    ...
    protected static function get_global_concepts() {
        ...
        if (is_array($data)) {
            ...
            $sql = "SELECT g.id, g.name
                      ...
                      JOIN {course_modules} cm ON (cm.instance = g.id)
                      JOIN {modules} m ON (m.name = 'glossarylv' AND m.id = cm.module)
                     WHERE g.usedynalink = 1 AND g.globalglossarylv = 1 AND cm.visible = 1 AND m.visible = 1
                  ORDER BY g.globalglossarylv, g.id";
     ...
     * @param int $courseid
     * @return array with two elements - array of glossaries and concepts for each glossarylv
     */
