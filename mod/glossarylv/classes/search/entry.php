...
namespace mod_glossarylv\search;
...
class entry extends \core_search\base_mod {
    ...
    /**
     * Returns recordset containing required data for indexing glossarylv entries.
     *
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        $sql = "SELECT ge.*, g.course FROM {glossarylv_entries} ge
    ...
    /**
     * Returns the documents associated with this glossarylv entry id.
     *
     * @param stdClass $entry glossarylv entry.
     * @param array    $options
    ...
    public function get_document($entry, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('glossarylv', $entry->glossarylvid, $entry->course);
            $context = \context_module::instance($cm->id);
        } catch (\dml_missing_record_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_glossarylv ' . $entry->id . ' document, not all required data is available: ' .
                $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        } catch (\dml_exception $ex) {
            // Notify it as we run here as admin, we should see everything.
            debugging('Error retrieving mod_glossarylv' . $entry->id . ' document: ' . $ex->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
     ...
     * @param int $id Glossarylv entry id
     * @return bool
     */
    public function check_access($id) {
        ...
        try {
            ...
            $cminfo = $this->get_cm('glossarylv', $entry->glossarylvid, $entry->course);
        } catch (\dml_missing_record_exception $ex) {
        ...
        if (!glossarylv_can_view_entry($entry, $cminfo)) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/glossarylv/view.php', $docparams);
    }

    /**
     * Link to the glossarylv.
     *
     * @param \core_search\document $doc
     * @return \moodle_url
     */
    public function get_context_url(\core_search\document $doc) {
        $contextmodule = \context::instance_by_id($doc->get('contextid'));
        return new \moodle_url('/mod/glossarylv/view.php', array('id' => $contextmodule->instanceid));
    }

    /**
     * Returns the specified glossarylv entry checking the internal cache.
     *
    ...
    protected function get_entry($entryid) {
        ...
        if (empty($this->entriesdata[$entryid])) {
            $this->entriesdata[$entryid] = $DB->get_record_sql("SELECT ge.*, g.course, g.defaultapproval FROM {glossarylv_entries} ge
                                                                  JOIN {glossarylv} g ON g.id = ge.glossarylvid
