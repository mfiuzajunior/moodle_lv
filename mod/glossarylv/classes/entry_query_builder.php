 ...
 * @since      Moodle 3.1
 */
class mod_glossarylv_entry_query_builder {

    /** Alias for table glossarylv_alias. */
    const ALIAS_ALIAS = 'ga';
    /** Alias for table glossarylv_categories. */
    const ALIAS_CATEGORIES = 'gc';
    /** Alias for table glossarylv_entries_categ. */
    const ALIAS_ENTRIES_CATEGORIES = 'gec';
    /** Alias for table glossarylv_entries. */
    ...
    protected $from;
    /** @var object The glossarylv we are fetching from. */
    protected $glossarylv;
     ...
     */
    public function __construct($glossarylv = null) {
        $this->from = sprintf('FROM {glossarylv_entries} %s', self::ALIAS_ENTRIES);
        if (!empty($glossarylv)) {
            $this->glossarylv = $glossarylv;
            $this->where[] = sprintf('(%s.glossarylvid = :gid OR %s.sourceglossarylvid = :gid2)',
                self::ALIAS_ENTRIES, self::ALIAS_ENTRIES);
            $this->params['gid'] = $glossarylv->id;
            $this->params['gid2'] = $glossarylv->id;
        }
    }
     ...
     * @param string $table The table name, without the prefix 'glossarylv_'.
    ...
    public function join_alias() {
        $this->joins[] = sprintf('LEFT JOIN {glossarylv_alias} %s ON %s = %s',
    ...
    public function join_category($categoryid) {
        ...
        if ($categoryid === GLOSSARYLV_SHOW_ALL_CATEGORIES) {
            $this->joins[] = sprintf('JOIN {glossarylv_entries_categ} %s ON %s = %s',
                self::ALIAS_ENTRIES_CATEGORIES, self::resolve_field('id', 'entries'),
                self::resolve_field('entryid', 'entries_categories'));

            $this->joins[] = sprintf('JOIN {glossarylv_categories} %s ON %s = %s',
            ...
            $this->joins[] = sprintf('LEFT JOIN {glossarylv_entries_categ} %s ON %s = %s',
                self::ALIAS_ENTRIES_CATEGORIES, self::resolve_field('id', 'entries'),
                self::resolve_field('entryid', 'entries_categories'));

        } else {
            $this->joins[] = sprintf('JOIN {glossarylv_entries_categ} %s ON %s = %s AND %s = :joincategoryid',
                self::ALIAS_ENTRIES_CATEGORIES, self::resolve_field('id', 'entries'),
                self::resolve_field('entryid', 'entries_categories'),
                self::resolve_field('categoryid', 'entries_categories'));
